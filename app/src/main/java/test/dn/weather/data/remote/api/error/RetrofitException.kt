...
open class RetrofitException : RuntimeException {
    ...
    constructor(type: String, cause: Throwable) : super(cause.message, cause) {
        this.errorType = type
    }
    ...
    fun getMsgErrorList(context: Context): String? {
        return when (errorType) {
    ...
    fun getAllMsgError(context: Context): String? {
        return when (errorType) {
    ...
    fun getHttpError(): String? {
        return errorResponse?.code?.getHttpErrorMessage()
    }
